...
class Tanggal {
    ...
    public static function toReadableMonth($date, $withSpan=TRUE){
        if($date == NULL) return "-";
        $withHour = TRUE;
        $arr = explode(" ", $date);
        if(count($arr)==1){
        ...
        $time = strtotime($date);
        $padTime = str_pad($time, 12, "0", STR_PAD_LEFT);
        return ($withSpan?"<span style='display:none'>{$padTime}</span>":"").($withHour?date(" Y, H:i", $time):Tanggal::getBulan(date($time)).date(" Y", $time));
    }
